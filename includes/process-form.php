...
// Check the nonce.
if ( ! isset( $_POST['camptix_csv_xml_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['camptix_csv_xml_nonce'] ) ), 'camptix_csv_xml_nonce' ) ) {
...
if ( $convert_2_xml ) {
	...
	if ( false !== $data_type ) {
		...
		echo '<a href="' . esc_url( CAMPTIX_XML_CSV_UPLOAD_URL . $xml_link ) . '" download="' . esc_attr( $xml_link ) . '">' . esc_html__( 'Download XML file', 'camptix-xml-csv' ) . '</a> <em>(' . esc_attr( $xml_link ) . ')</em></div>';
	} else {
